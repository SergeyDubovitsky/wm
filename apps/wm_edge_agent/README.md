...
uv run --env-file .env --package wm-edge-agent wm-edge-agent \
  check-config --bootstrap-config environments/demo-stand/wm_edge_agent/bootstrap.yaml
uv run --env-file .env --package wm-edge-agent wm-edge-agent \
  show-config --bootstrap-config environments/demo-stand/wm_edge_agent/bootstrap.yaml --format json
uv run --env-file .env --package wm-edge-agent wm-edge-agent \
  enqueue-demo-event --bootstrap-config environments/demo-stand/wm_edge_agent/bootstrap.yaml
uv run --env-file .env --package wm-edge-agent wm-edge-agent \
  deliver-once --bootstrap-config environments/demo-stand/wm_edge_agent/bootstrap.yaml
```
...
- `../../docs/contracts/wm-edge-agent/` — canonical edge contracts, MQTT topic tree, and payload schemas
...
- `../../environments/demo-stand/wm_edge_agent/` — demo-stand bootstrap и config bundle
