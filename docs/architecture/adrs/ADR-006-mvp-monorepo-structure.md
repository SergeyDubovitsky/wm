...
- `apps/wm_edge_agent/` — основной runtime edge-агента на Python
- `apps/wm_knx_demo/` — Python-утилиты и сценарии для demo-стенда
- `libs/wm_knx_parser/` — Python-библиотека для разбора `ETS .knxproj`
...
- `libs/wm_knx_parser/`
- `libs/wm_demo_stack/`
- `tools/wm_clickhouse_migrations/`
...
- `wm_knx_parser` остается в `libs/`, потому что это самостоятельный Python API для
  чтения `ETS .knxproj`, а CLI является оберткой поверх библиотечной функции
- `wm_demo_stack` тоже остается в `libs/`, потому что его модели и генераторы
...
- `wm-clickhouse` живет в `tools/`, потому что его primary role — migration и
...
Если `wm_edge_agent` и другие приложения запускаются на хосте через `uv`, а
...
`MQTT broker` и integration-тестами `KNX -> wm_edge_agent -> MQTT`.

Это решение фиксирует текущую границу MVP wm-edge-agent:

- smoke-test публикации событий из `wm_edge_agent` выполняется через MQTT tests
...
- Grafana остается целевым visualization layer платформы, но не входит в
  обязательный локальный demo surface wm-edge-agent
...
- `apps/wm_edge_agent/config/examples/bootstrap.example.yaml`
...
- `apps/wm_edge_agent` — основной production-oriented runtime
- `apps/wm_knx_demo` — demo и инженерные сценарии для стенда
- `libs/wm_knx_parser` — самостоятельная библиотека, которую могут использовать
...
- Сразу разнести `wm_edge_agent`, `wm_knx_demo` и `wm_knx_parser` по отдельным
...
2. `wm_edge_agent` и `wm_knx_demo` перенесены в `apps/`.
3. `wm_knx_parser` перенесен в `libs/`.
