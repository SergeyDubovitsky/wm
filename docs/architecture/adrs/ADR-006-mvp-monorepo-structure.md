...
- конфиги `MQTT broker`
- provisioning и конфиги `Grafana`
- при необходимости дополнительные локальные сервисы вроде тестового БД,
  consumer-а или observability-компонентов
...
- `apps/edge_agent/config/examples/sources.d/*.yaml`
- `apps/edge_agent/config/examples/points.d/*.yaml`
- `infra/local/mosquitto/mosquitto.conf`
- `infra/local/grafana/provisioning/*`
...
#### 7.2. Environment-specific runtime configs

Конкретные runtime-конфиги для стенда, объекта или dev-окружения хранятся
...
- здесь лежат не “примеры”, а конкретные конфиги запуска
...
- такие конфиги можно коммитить только если в них нет секретов и чувствительных
  endpoint-данных
- именно эти файлы должны монтироваться в контейнеры или передаваться в
  приложения при локальном запуске

Для `edge_agent` это особенно важно: шаблоны формата принадлежат
`apps/edge_agent`, а конфиг конкретного объекта принадлежит
`environments/<site>/edge_agent/`.
...
- `.env.local`
- `.secrets/`
- локальные credentials для `MQTT`, `Grafana`, БД и внешних API
...
- `infra/local` — локальная среда разработки с `MQTT broker`, `Grafana` и
  другими общими сервисами
- `environments/*` — конкретные runtime-конфиги стендов, объектов и локальных
  профилей запуска
...
- локальная инфраструктура получает собственное предсказуемое место в
  репозитории
- конкретные runtime-конфиги стендов получают отдельное предсказуемое место и
...
5. Следующим шагом остается добавить `infra/local/` и environment-specific runtime configs.
