...
Для промышленной системы конфигурация edge-agent должна управляться платформой
...
На первом этапе authoring source of truth — versioned YAML config bundle в
репозитории или operations bundle. Отдельный config publisher tool валидирует
YAML bundle по контрактам, формирует retained MQTT payloads и публикует их в
broker. Server UI/API не реализуется на этом этапе.

Целевой будущий source of truth — `Platform Store` и `Platform API`. Переход на
API должен сохранить MQTT wire contracts.
...
- edge-agent больше не требует локального runtime registry точек
...
- нужен отдельный config publisher tool и дисциплина публикации revisions
...
Точные схемы и topic names находятся в `docs/contracts/edge-agent/`,
