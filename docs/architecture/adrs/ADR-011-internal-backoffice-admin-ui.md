# ADR-011: Internal backoffice admin UI для Platform Config API
...
`ADR-010` фиксирует первый backend-срез платформы: `Platform Config API` на
...
Админка подключается как infrastructure adapter внутри `apps/platform_config_api`.
...
2. Подключить SQLAdmin в `apps/platform_config_api` на `/backoffice`.
