...
Production-модель после `ADR-008`: edge-agent стартует с минимальным
...
- `tenant_id` приходит в edge-agent из `wm.edge.runtime-config.v1` и публикуется в `wm.telemetry.event.v1` payload как claim; ingestion обязан валидировать claim.
...
- YAML config bundle является временным authoring/import path до внедрения `Config Registry`; после `ADR-010` source of truth переезжает в `Platform Store` без изменения wire contracts.
